...
use crate::{
    ...
    cyclotomic_ring::{
        CRT, Cyclotomic, CyclotomicConfig, CyclotomicPolyRingGeneral,
        CyclotomicPolyRingNTTGeneral, ICRT,
    },
...
pub type RqNTT =
    CyclotomicPolyRingNTTGeneral<GoldilocksRingConfig, 1, { ntt::N }>;
pub type RqPoly =
    CyclotomicPolyRingGeneral<GoldilocksRingConfig, 1, { ntt::D }>;
...
impl Ring for Fq3 {
    const ZERO: Self = <Fq3 as AdditiveGroup>::ZERO;
...
mod test {
    ...
    fn test_reduce() {
        ...
        let mut coeffs: Vec<Fq> =
            (0..(2 * ntt::D)).map(|_| Fq::rand(&mut rng)).collect();
        ...
        let (_, r) = DenseOrSparsePolynomial::divide_with_q_and_r(
            &poly.into(),
            &the_cyclotomic.into(),
        )
        .unwrap();
