...
impl RingElement {
    /// Ensures two elements share the same dimension.
    fn ensure_same_dimension(&self, other: &RingElement) -> Result<()> {
    ...
    /// Resolves a compatible modulus between two elements.
    fn combined_modulus(&self, other: &RingElement) -> Result<Option<i64>> {
        match (self.modulus, other.modulus) {
            (Some(q1), Some(q2)) if q1 != q2 => {
                Err(CyclotomicRingError::IncompatibleModuli {
                    modulus1: q1,
                    modulus2: q2,
                })
            }
            (Some(q1), Some(_)) => Ok(Some(q1)),
            (Some(q), None) | (None, Some(q)) => Ok(Some(q)),
            (None, None) => Ok(None),
        }
    }
    ...
    pub fn zero(dimension: usize, modulus: Option<i64>) -> Result<Self> {
        ...
        let coefficients =
            BalancedCoefficients::with_dimension(dimension, modulus_value)?;
    ...
    pub fn from_coefficients(
        ...
        let coefficients =
            BalancedCoefficients::new(coeffs.to_vec(), modulus_value)?;
    ...
    pub fn divide(&self, _other: &Self) -> Result<Self> {
    ...
    pub fn mul(&self, other: &RingElement) -> Result<RingElement> {
        ...
        if self.modulus != other.modulus {
            return Err(CyclotomicRingError::InvalidParameters(
                "Ring elements must have the same modulus for multiplication"
                    .to_string(),
            ));
        }
    ...
    pub fn add(&self, other: &RingElement) -> Result<RingElement> {
        ...
        if self.modulus != other.modulus {
            return Err(CyclotomicRingError::InvalidParameters(
                "Ring elements must have the same modulus for addition"
                    .to_string(),
            ));
        }
    ...
    fn multiply_schoolbook(&self, other: &Self) -> Result<Self> {
        ...
        for i in 0..self.dimension {
            for j in 0..self.dimension {
                let coeff_product = (self.coefficients.coeffs[i] as i128)
                    * (other.coefficients.coeffs[j] as i128);
                let power = i + j;

                if power < self.dimension {
                    // Normal case: X^{i+j} where i+j < d
                    result_coeffs[power] =
                        result_coeffs[power].wrapping_add(coeff_product as i64);
                } else {
                    // Reduction case: X^{i+j} = -X^{i+j-d} where i+j ≥ d
                    let reduced_power = power - self.dimension;
                    result_coeffs[reduced_power] = result_coeffs[reduced_power]
                        .wrapping_sub(coeff_product as i64);
                }
            }
        }
    ...
    fn multiply_karatsuba(&self, other: &Self) -> Result<Self> {
        ...
        let result = self.karatsuba_recursive(
    ...
    fn karatsuba_recursive(
        ...
        let a1 = if half < a.len() {
            &a[half..n.min(a.len())]
        } else {
            &[]
        };
        let b0 = &b[..half.min(b.len())];
        let b1 = if half < b.len() {
            &b[half..n.min(b.len())]
        } else {
            &[]
        };
        ...
        for i in 0..half {
            a_sum[i] = a0.get(i).copied().unwrap_or(0)
                + a1.get(i).copied().unwrap_or(0);
            b_sum[i] = b0.get(i).copied().unwrap_or(0)
                + b1.get(i).copied().unwrap_or(0);
        }
    ...
    pub fn multiply_ntt(&self, other: &Self) -> Result<Self> {
        ...
        let modulus = self.modulus.or(other.modulus).ok_or_else(|| {
            CyclotomicRingError::InvalidParameters(
                "No modulus available for NTT multiplication".to_string(),
            )
        })?;
        ...
        for (&a, &b) in self
            ...
            .coeffs
            .iter()
            .zip(other.coefficients.coeffs.iter())
        {
            let mut reduced =
    ...
    fn schoolbook_multiply(&self, other: &RingElement) -> Result<Vec<i64>> {
        ...
        for i in 0..d {
            for j in 0..d {
                let coeff_product =
                    (self_coeffs[i] as i128) * (other_coeffs[j] as i128);
                let degree_sum = i + j;

                if degree_sum < d {
                    // Normal case: X^i * X^j = X^{i+j}
                    result[degree_sum] =
                        result[degree_sum].wrapping_add(coeff_product as i64);
                } else {
                    // Cyclotomic reduction: X^{i+j} = X^{i+j-d} * X^d = -X^{i+j-d}
                    result[degree_sum - d] = result[degree_sum - d]
                        .wrapping_sub(coeff_product as i64);
                }
            }
        }
    ...
    #[allow(dead_code)]
    pub(crate) fn karatsuba_multiply(
        &self,
        other: &RingElement,
    ) -> Result<Vec<i64>> {
    ...
    pub fn validate(&self) -> Result<()> {
        ...
        if self.dimension < MIN_RING_DIMENSION
            || self.dimension > MAX_RING_DIMENSION
        {
            return Err(CyclotomicRingError::InvalidDimension {
...
#[allow(dead_code, clippy::needless_range_loop)]
pub(crate) fn schoolbook_multiply(
    f: &RingElement,
    g: &RingElement,
) -> Result<RingElement> {
                ...
                if (i + j) % d == k {
                    if i + j < d {
                        ...
                        sum = sum.wrapping_add(
                            f_coeffs[i].wrapping_mul(g_coeffs[j]),
                        );
                    } else {
...
pub(crate) fn karatsuba_multiply(
    ...
    let f_low =
        RingElement::from_coefficients(&f_coeffs[..half_d], result_modulus)?;
    let f_high =
        RingElement::from_coefficients(&f_coeffs[half_d..], result_modulus)?;
    let g_low =
        RingElement::from_coefficients(&g_coeffs[..half_d], result_modulus)?;
    let g_high =
        RingElement::from_coefficients(&g_coeffs[half_d..], result_modulus)?;
    ...
    let p2 = RingElement::sub(&p2_full, &p1.clone())?;
    let p2 = RingElement::sub(&p2, &p3.clone())?;
...
impl RingElement {
    ...
    pub fn negate(&self) -> Result<RingElement> {
        ...
        for &coeff in coeffs {
            ...
            let balanced_negated =
                RingConfig::reduce_to_balanced(negated, modulus);
