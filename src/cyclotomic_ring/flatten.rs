...
use super::{
    CyclotomicConfig, CyclotomicPolyRingGeneral, CyclotomicPolyRingNTTGeneral,
};
...
mod tests {
    ...
    fn test_flatten_promote_coeff() {
        ...
        let orig: Vec<RqPoly> =
            (0..3).map(|_| RqPoly::rand(&mut rng)).collect();
