...
mod tests {
    use super::*;
    use crate::cyclotomic_ring::models::goldilocks;
    ...
    macro_rules! test_crt_conversion {
        ($poly_type:ty, $ntt_type:ty, $test_name:ident, $test_vec_name:ident) => {
            ...
            fn $test_vec_name() {
                let mut rng = test_rng();
                for _ in 0..100 {
                    let original_vec: Vec<$poly_type> = (0..100)
                        .map(|_| <$poly_type>::rand(&mut rng))
                        .collect();

                    // Convert to NTT form using CRT trait
                    let ntt_form: Vec<$ntt_type> =
                        CRT::elementwise_crt(original_vec.clone());

                    // Convert back to PolyRing using ICRT trait
                    let converted_back_vec: Vec<$poly_type> =
                        ICRT::elementwise_icrt(ntt_form);
