use ark_std::{
    One, Zero, cfg_chunks, cfg_chunks_mut, cfg_into_iter, cfg_iter,
    cfg_iter_mut,
    iter::Sum,
...
use crate::{
    PolyRing, Ring,
    linalg::{
        Matrix, RowVector, SymmetricMatrix, Transpose, Vector, rounded_div,
    },
};
...
pub fn decompose_balanced_in_place<R: ConvertibleRing>(
    v: &R,
    b: u128,
    out: &mut [R],
) {
    ...
    R::BaseRing: Decompose,
{
    cfg_iter!(v)
        .map(|ring_elem| {
            decompose_balanced_polyring(ring_elem, b, padding_size)
        })
        .collect()
}
    ...
    fn test_decompose_balanced_vec() {
        ...
        for b in BASIS_TEST_RANGE {
            ...
            for i in 0..decomp.len() {
                // Check that the decomposition is correct
                let decomp_i =
                    decomp.iter().map(|d_j| d_j[i]).collect::<Vec<_>>();
